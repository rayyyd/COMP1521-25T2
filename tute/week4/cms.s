...
find_student_by_id__prologue:
	begin
	#$a0 = id
find_student_by_id__for_init:
	li	$t0, 0
find_student_by_id__for_cond:
	bge	$t0, CLASS_SIZE, find_student_by_id__for_end
	# students[i].id
	# get students[i] first
	mul	$t1, $t0, SIZEOF_STRUCT_STUDENT
	addi	$t1, students
	# add on the id offset.
	add	$t2, $t1, STRUCT_STUDENT_ID_OFFSET
	lw	$t2, ($t2)

	bne     $t2, $a0, find_student_by_id__if_end 
	move	$v0, $t1
	j	find_student_by_id__epilogue
find_student_by_id__if_end:
	addi	$t0, 1
	j	find_student_by_id__for_cond
find_student_by_id__for_end:
	li	$v0, NULL
